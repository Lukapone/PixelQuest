...
pub fn main() !void {
    ...
    // const windowx = c.glfwCreateWindow(
    ...
    // defer c.glfwDestroyWindow(windowx);

    // Create our window
    const window = glfw.Window.create(extent.width, extent.height, "PixelQuest", null, null,
     .{.client_api = .no_api},)
    orelse {
    ...
    extent.width, extent.height = blk: {
        ...
        const initialSize = window.getFramebufferSize();
        break :blk .{ initialSize.width, initialSize.height };
    };
    ...
    const gc = try GraphicsContext.init(allocator, app_name, window.handle);
    defer gc.deinit();
    ...
    const sizeBasic = window.getSize();
    log.info("Window size: {d}x{d}", .{ sizeBasic.width, sizeBasic.height });
